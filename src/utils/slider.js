...
export const generateSlideObject = (content, slide_id, audio) => {
  ...
  try {
    if (!!slide) {
      ...
      if (!!styles) {
        ...
        if (styles.hasOwnProperty("background-color")) {
          if (styles["background-color"].startsWith("#")) {
            ...
            slideParams.isBackgroundSet = true;
          }
        ...
        .forEach(field => {
          ...
          if (slideType === "iframe") {
            ...
            if (!!imgTag) {
              ...
              slideParams.isBackgroundSet = false;
            }
          }
          ...
          if (slideType === "gif") {
            ...
            if (!!imgTag) {
              const imgUrl = getValue(imgTag.attributes, "src");
              const gifUrl = getValue(imgTag.attributes, "data-video-url");
              slideParams.gifUrl = gifUrl;
              slideParams.videoPreviewImageUrl = imgUrl;
              slideParams.isBackgroundSet = false;
            }
          }
      ...
      if (audio) {
        slideParams.audioUrl = audio.audio_url;
        slideParams.audioDuration = audio.audio_duration;
        slideParams.audioAutoplay = audio.audio_autoplay;
      }
...
export const generateGifSlideString = url =>
  `<div data-block-type="gif" style="position : relative; height: 100%; width: 100%;"><div style="width: 100%; height: 100%;"><img src="${url}" data-video-url="${url}" /><a href="${url}"></a></div></div>`;
  ...
  return `
    ...
    obj.backgroundType === "color" && obj.backgroundColor
      ...
      : `background-image: url(${
          obj.gifUrl ? obj.gifUrl : obj.backgroundImage
        }); background-position: ${
          obj.gifUrl ? "center" : obj.backgroundPosition
        }; background-size: ${
          obj.gifUrl ? "cover" : obj.backgroundSize
        }; background-repeat: ${
          obj.gifUrl ? "no-repeat" : obj.backgroundRepeat
        };`
  }">${
    !!obj.videoUrl
      ? generateVideoSlideString(obj.videoUrl, obj.videoPreviewImageUrl)
      : obj.gifUrl
      ? generateGifSlideString(obj.gifUrl)
