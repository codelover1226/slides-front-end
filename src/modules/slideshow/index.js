...
const Slideshow = () => {
  ...
  const [musicButtonHover, setMusicButtonHover] = useState(false);
  ...
  const handleHover = () => {
    setMusicButtonHover(!musicButtonHover);
  };
  ...
  useEffect(() => {
    if (!!settings && !loading) {
      ref.current = new Reveal({
        slideNumber: isPreviewRoute ? "c/t" : false, //slideNumber: settings.slideNumbers && isPreviewRoute ? "c/t" : false,
  ...
  //       });
  //       slideAudio[currentIndex]?.audio.play();

  //       slideAudio[currentIndex].audio.addEventListener("ended", () => {
  //         slideAudio[currentIndex]?.audio.play();
  //       });
  //     } else {
  //       slideAudio[currentIndex]?.audio.pause();
  //     }

  //     !isEmptyArray(sources) && sources[audioIndex].audio.pause();
  //   } else if (!isEmptyArray(sources)) {
  //     slideAudio[currentIndex].currentTime = 0;
  //     slideAudio?.forEach(singleAudio => {
  //       singleAudio.audio.pause();
  //       singleAudio.audio.currentTime = 0;
  //     });
  ...
  //           ? sources[audioIndex].audio.play()
  //           : setAudioIndex(0);
  //         // setAudioIndex(sources.length === audioIndex + 1 ? 0 : audioIndex + 1);
  //       } else {
  ...
  return (
    <div className="main_part">
      <div className="reveal slider_preview">
        ...
        <div className="modal-footer-wrapper">
          {
            isPreviewRoute && !isEmptyArray(slideAudio) && (
              // slideAudio[currentIndex]?.url !== "" && (
              <div
            ...
            ) : isAudioPlaying && isPlaylistPlaying ? (
              <i
              ...
              ref={playerRef}
              autoPlay={true}
              src={slideAudio[currentIndex]?.url}
          ...
          <div className="maximize-container" onClick={handleFullscreen}>
            {isFullScreen ? (
              <img
                src={maximizeIcon}
                alt="zoomIn"
